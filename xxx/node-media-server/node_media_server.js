...
class NodeMediaServer {
    ...
    Https.get('https://registry.npmjs.org/node-media-server', function (res) {
      ...
      res.on('end', function () {
        ...
        let latestVersionNum =
          (latestVersion.split('.')[0] << 16) | (latestVersion.split('.')[1] << 8) | (latestVersion.split('.')[2] & 0xff);
        let thisVersionNum =
          (Package.version.split('.')[0] << 16) | (Package.version.split('.')[1] << 8) | (Package.version.split('.')[2] & 0xff);
  ...
  startArchiving() {
    if (!this.nts) {
      console.error('Node Transport Server is dead!');
      return;
    }

    this.nts.startArchiving();
