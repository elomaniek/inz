...
class NodeTransServer {
  ...
  startArchiving() {
    this.config.dumping = true;
    this.transSessions.forEach(session => session.startDumping());
  }

  stopArchiving() {
    this.config.dumping = false;
    this.transSessions.forEach(session => session.stopDumping());
  }

  stop() {
    this.transSessions.forEach(session => session.end());
    this.transSessions.clear();
  }
  ...
  onPostPublish(id, streamPath, args) {
    ...
    while (i--) {
      ...
      conf.args = args;
      conf.dumping = this.config.dumping;
      if (app === conf.app) {
